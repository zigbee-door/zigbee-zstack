...
const pTaskEventHandlerFn tasksArr[] = {      //taskArr指向了各任务事件的处理函数
  ...
  ZDNwkMgr_event_loop,                    
#endif
  LockApp_ProcessEvent                        //门锁无线通信任务,优先级最低
};
...
void osalInitTasks( void )
{
  ...
  macTaskInit( taskID++ );            //MAC层任务初始化,任务优先级最高 
  nwk_init( taskID++ );               //网络层任务初始化 
  ...
  APSF_Init( taskID++ );
#endif
  ZDApp_Init( taskID++ );             //ZDAPP任务初始化
#if defined ( ZIGBEE_FREQ_AGILITY ) || defined ( ZIGBEE_PANID_CONFLICT )
  ZDNwkMgr_Init( taskID++ );
#endif
  LockApp_Init( taskID );             //门锁无线通信任务初始化
}
