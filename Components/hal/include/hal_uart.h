...
typedef struct
{
  ...
  uint16 maxBufSize;                        //最大数据长度
  uint8 *pBuffer;                           //传输的数据
} halUARTBufControl_t;

typedef struct
{
  bool                configured;           //是否设置串口，为TRUE时配置串口
  uint8               baudRate;             //波特率
  bool                flowControl;          //控制流设置，TRUE时流控制
  uint16              flowControlThreshold; //在RX缓存达到maxRxBufSize之前的空余字节数
  uint8               idleTimeout;          //
  halUARTBufControl_t rx;                   //rx和tx通过串口发送的数据及数据长度
  halUARTBufControl_t tx;
  bool                intEnable;            //使能串口，设置为true时打开串口
  uint32              rxChRvdTime;          //
