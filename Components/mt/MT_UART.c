...
void MT_UartInit ()
{
  ...
  uartConfig.baudRate             = MT_UART_DEFAULT_BAUDRATE; //波特率9600
  uartConfig.flowControl          = MT_UART_DEFAULT_OVERFLOW; //配置流控，如果只连接了TX/RX 2根线，一定要关流控，否则无法收发信息
  uartConfig.flowControlThreshold = MT_UART_DEFAULT_THRESHOLD;
  uartConfig.rx.maxBufSize        = MT_UART_DEFAULT_MAX_RX_BUFF;
  uartConfig.tx.maxBufSize        = MT_UART_DEFAULT_MAX_TX_BUFF;
  uartConfig.idleTimeout          = MT_UART_DEFAULT_IDLE_TIMEOUT;
  uartConfig.intEnable            = TRUE;
#if defined (ZTOOL_P1) || defined (ZTOOL_P2)  //查看工程的option，C/C++ Preprocesser ZTOOL_P1 P1和P2代表串口1或者串口2
