...
void osal_start_system( void )
{
...
#endif
  {
    ...
    //（对于本例子来说，任务数组里只有七个任务，tasksEvents[0]～tasksEvents[6]，
    //tasksEvents[6]就是用户自已添加的任务，idx随着用户添加任务的增多而增大）
    ...
    if (idx < tasksCnt)
    {
      ...
      HAL_ENTER_CRITICAL_SECTION(intState);
      tasksEvents[idx] |= events;               //把刚才返回未处理的任务事件添加加当前任务中再进行处理
      HAL_EXIT_CRITICAL_SECTION(intState);      //（跳出此if(idx < tasksCnt)循环再进行if (tasksEvents[idx])判断并处理）
    }
